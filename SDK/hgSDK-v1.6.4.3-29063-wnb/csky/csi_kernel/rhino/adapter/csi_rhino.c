...
extern void krhino_task_dump(ktask_t *task);
...
void csi_kernel_task_dump(k_task_handle_t task_handle)
{
    krhino_task_dump(task_handle);
}
...
uint32_t csi_kernel_task_runtime(struct os_task_time *tsk_rumtime, int count)
{
#if (RHINO_CONFIG_SYSTEM_STATS > 0)
    uint32_t ret = 0;
    klist_t *taskhead;
    ...
    for (tmp = taskhead->next; tmp != taskend && ret < count; tmp = tmp->next) {
        ...
        tsk_rumtime[ret].name = (const uint8_t *)task->task_name;
