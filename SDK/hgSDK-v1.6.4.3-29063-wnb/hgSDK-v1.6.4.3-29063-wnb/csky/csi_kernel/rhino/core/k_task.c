                        ...
                        uint8_t cpu_num, uint8_t cpu_binded, uint8_t autorun)
{
    ...
    // task_obj = krhino_mm_alloc(sizeof(ktask_t) + stack * sizeof(cpu_stack_t), __builtin_return_address(0));
    // if (task_obj == NULL) {
...
void krhino_task_dump(ktask_t *task)
{
    int i = 0;
    uint32_t *addr = 0;
    hgprintf("Task:%s\r\n", task->task_name);
    ...
    hgprintf("    task_stack: 0x%08x,0x%08x,0x%08x\r\n", (uint32_t)task->task_stack_base, (uint32_t)task->task_stack, (uint32_t)(task->task_stack_base + task->stack_size));
    hgprintf("    stack dump:\r\n    ");
    for(addr=task->task_stack; addr<task->task_stack_base+task->stack_size; addr++){
